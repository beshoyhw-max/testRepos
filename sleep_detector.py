...
class SleepDetector:
    def __init__(self, pose_model_path='yolo11n-pose.pt', mp_model_path='models/face_landmarker.task', pose_model_instance=None):
        ...
        # State: {track_id: {'closed_start': time, 'status': 'awake'}}
        # Since we might process single crops without tracking IDs passed in yet,
        # we might need to rely on the caller to manage state or assume single person per camera for now.
        # For this iteration, we will implement stateless detection per frame and let the caller/manager handle temporal smoothing if needed,
        # BUT "sleeping" requires time. So we DO need state.
        # We'll use a simple dictionary mapping 'camera_name' (or passed ID) to state.
        self.state = {}

    def process_crop(self, crop, id_key="unknown"):
        """
        Analyzes a person crop for sleep signs.
        Returns: (status_code, debug_info)
        status_code: 'awake', 'drowsy', 'sleeping', 'posture_sleep'
        """
        if crop.size == 0:
            return "awake", {}
        ...
        if len(pose_results) > 0 and pose_results[0].keypoints is not None:
            ...
            if has_pt(0) and has_shoulders:
                ...
                # If Nose is significantly below shoulders (coordinate Y increases downwards)
                # This is tricky as camera angle varies. Let's be conservative.
                # If nose is below shoulders, they are bent over.
                if nose_y > shoulder_y:
                    is_posture_sleep = True

        if is_posture_sleep:
            return "sleeping", {"reason": "posture"}

        # --- 2. EYES CHECK (MediaPipe) ---
        # Convert to MP Image
        ...
        if not detection_result.face_landmarks:
            # Face not found by MP (but found by YOLO? might be occlusion)
            return "awake", {"reason": "no_face_mp"}
        ...
        if avg_ear < self.EAR_THRESHOLD:
            ...
            if duration > self.SLEEP_TIME_THRESHOLD:
                return "sleeping", {"ear": avg_ear, "duration": duration}
            else:
                return "drowsy", {"ear": avg_ear, "duration": duration}
        else:
            # Eyes Open
            state['closed_start'] = None
            return "awake", {"ear": avg_ear}
    ...
    def _calculate_ear(self, landmarks, indices):
        ...
        def dist(i1, i2):
            p1 = landmarks[i1]
            p2 = landmarks[i2]
            # landmarks are normalized (0-1). We can use them directly for ratio.
